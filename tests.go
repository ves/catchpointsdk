...
func AddTest(folder_name string, product_name string, test *TestPayload) string {
  fmt.Println(GetDefaultDivisionId())
  token := Authenticate()
  ...
  if folder_name != "" { test.ParentFolderID = GetFolderIdByName(folder_name) }
  if product_name == "" {
...
func listTests() []byte {
  ...
  token := Authenticate()
  gr := gorequest.New()
  resp, body, errs := gr.Get(fmt.Sprintf("%s/ui/api/v1/tests?division_id=%s", c.Endpoint, division_id)).
    Set("Accept", "*/*").
    Set("Authorization", fmt.Sprintf("Bearer %s", token)).
    EndBytes()
